...
The rules reject TypeScript SQL template interpolation, C# `BinaryFormatter` deserialization, and logging likely secret variables. The files under `tests/semgrep` intentionally trigger all rules. This repository ignores everything by default, so CI copies the fixtures to a temporary directory before scanning them; production code should produce no findings.
